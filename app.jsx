...
import { CheckCircle2, FileText, Loader2, Sparkles, AlertCircle, Download, User, RefreshCcw } from 'lucide-react';
...
// Securely access API key from Vite environment variables
...
export default function App() {
  ...
  const callGemini = async (history) => {
    try {
      if (!GENAI_API_KEY) {
        throw new Error("API Key logic: VITE_GEMINI_API_KEY is not set in .env");
      }
  ...
  const handleAnswerSubmit = async (blockId, answerText, forceSummary = false) => {
    ...
    try {
      ...
      // Revert the last lock if it failed to allow retry
      setBlocks(blocks);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-gray-800 font-sans selection:bg-blue-100">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 pb-32">
        ...
        <div className="space-y-12">
          {blocks.map((block, index) => (
            block.type === 'qa' ? (
              ...
              <SummaryBlock key={block.id} summary={block.data} />
            )
          ))}
        </div>
        ...
        {error && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-red-700 flex flex-col gap-3">
            <div className="flex items-center gap-2 font-bold">
              <AlertCircle className="w-5 h-5" /> Connection Issue
            </div>
            ...
            >
              <RefreshCcw className="w-3 h-3" /> Refresh Session
            </button>
          </div>
        )}
        <div ref={endOfDocRef} />
